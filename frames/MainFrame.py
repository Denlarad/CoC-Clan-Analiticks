...
class MainFrame:
    ...
    async def create(self, mail=None):
        ...
        await self.client.create(self.mail, keyring.get_password("OOOMEGALUL", self.mail))
    ...
    async def search(self, isUpdate):
        ...
        if isUpdate:
            if self.lastSearchedClan["tag"] == "":
                ...
                return False
            resData = await self.client.getClanInfo(self.lastSearchedClan["tag"])
        else:
            resData = await self.client.getClanInfo(self.clanTagEntry.get())
        ...
        CWL = await self.client.getWarLeagueInfo(resData.tag)
        if CWL:
            self.loadingFrame.nextStep("Сохранение информации по ЛКВ")
            await dataProceesing.saveLeagueWarData(CWL[0], CWL[1], resData.tag,self.client)

        self.loadingFrame.nextStep("Поиск информации по Столице")
        raids = await self.client.getCapitalRaids(resData.tag)
        ...
        await self.warFrame.updateData(self.loadingFrame)
        await self.searchFrame.updateData()
        await self.leagueFrame.updateData(self.loadingFrame)
        self.clanGamesFrame.updateData()
        await self.capitalFrame.updateData(self.loadingFrame)
